...
//    scene->addWidget(ph);
    ...
    phTE->setGeometry(74,8, 35,19);
    phTE->setText("0000");
    phTE->setValidator(new QRegExpValidator(QRegExp("[0-9a-f]{0,4}"), 0));
...
//    QObject::connect(X, SIGNAL(valueChanged()), this, SLOT(slotRegisterChanged()));
    ...
    phTE->setFont(QFont(phTE->font().family(), phTE->font().pointSize() - 2));
    phTE->setAlignment(Qt::AlignVCenter | Qt::AlignLeft);
    phTE->setGeometry(170,8, 70,19);
    phTE->setText("0x000000");
    phTE->setValidator(new QRegExpValidator(QRegExp("0x([0-9a-f]){0,6}"), 0));
    phTE->setPalette(QPalette(Qt::white));
    scene->addWidget(phTE);
    ...
    phTE->setFont(QFont(phTE->font().family(), phTE->font().pointSize() - 2));
    phTE->setAlignment(Qt::AlignVCenter | Qt::AlignLeft);
    phTE->setGeometry(170,30, 35,19);
    phTE->setText("0x00");
    phTE->setValidator(new QRegExpValidator(QRegExp("0x([0-9a-f]){0,2}"), 0));
    phTE->setPalette(QPalette(Qt::white));
    scene->addWidget(phTE);
